...
export function getIntroVideoFields(type) {
  const fields = [
    ...
    'title',
    'brand',
    `"instructor": *[_type == "method-v2" && brand == ^.brand && references(^._id)].${instructorField}`,
    `"difficulty": *[_type == "method-v2" && brand == ^.brand && references(^._id)].difficulty`,
    `"difficulty_string": *[_type == "method-v2" && brand == ^.brand].difficulty_string`,
    `"type": _type`,
    `"description": ${descriptionField}`,
    `"thumbnail": thumbnail.asset->url`,
    'length_in_seconds',
