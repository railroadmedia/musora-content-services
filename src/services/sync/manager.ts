...
import { contentProgressActivityObserver } from '../user-activity/content-progress-activity-observer.js'
...
export default class SyncManager {
  ...
  setup() {
    ...
    contentProgressActivityObserver.start(database).catch((error) => {
      this.telemetry.error('[SyncManager] Failed to start contentProgressActivityObserver', error)
    })
    ...
    const teardown = async (mode: SyncTeardownMode = 'reset') => {
      ...
      this.teardownPromise = (async () => {
        ...
        try {
          ...
          contentProgressActivityObserver.stop()
        } catch (error) {
